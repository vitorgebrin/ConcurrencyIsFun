...
class DownloadImageAsyncImageLoader {
    ...
    func downloadWithAsync() async throws -> UIImage?{ // Async functions need to be marked with the async keyword
        do {
            let (data,response) = try await URLSession.shared.data(from: url) // since this is a asynchronous function, I need to wait for the result. To do that, we use the keyword await
